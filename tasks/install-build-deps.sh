...
if [[ $TRAVIS_OS_NAME == 'linux' ]]; then
    ...
    sudo apt-get update
    sudo -E apt-get -yq --no-install-suggests --no-install-recommends install gcc-multilib g++-multilib rpm libopenjp2-tools
    ...
    sudo apt-get update
    sudo apt-get install --install-recommends -y wine
fi
